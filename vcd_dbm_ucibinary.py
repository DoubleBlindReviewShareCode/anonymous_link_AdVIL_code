...
# data and model
settings = {
    'adult-1': (123, 50, 15, [25], [], [], 'sigmoid', 'adult', 500),
    'connect4-1': (126, 50, 15, [25], [], [], 'sigmoid', 'connect4', 200),
    'digits-1': (64, 50, 15, [25], [], [], 'sigmoid', 'digits', 200),
    'dna-1': (180, 50, 15, [25], [], [], 'sigmoid', 'dna', 200),
    'mushrooms-1': (112, 50, 15, [25], [], [], 'sigmoid', 'mushrooms', 200),
    'nips-1': (500, 200, 50, [100], [], [], 'sigmoid', 'nips', 10),
    'ocr_letters-1': (128, 50, 15, [50], [], [], 'sigmoid', 'ocr_letters', 200),
    'rcv1-1': (150, 50, 15, [25], [], [], 'sigmoid', 'rcv1', 200),
    'web-1': (300, 100, 25, [50], [], [], 'sigmoid', 'web', 200),
}
...
enc = SBNENC(dim_h1v[::-1], dim_h2h1[::-1], activation_fn=tf.nn.sigmoid, temp=.1, hard=False)

def loss_rbm(real_v):
    _, real_h1, real_h2 = enc.log_conditional_prob(real_v)
    positve_phase = tf.reduce_mean(dbm.energy(real_h2, real_h1, real_v), axis=0)
    fake_v = dbm.cfe_step(real_v, enc, train_mc_steps)
    _, fake_h1, fake_h2 = enc.log_conditional_prob(fake_v)
...
train loop
'''
saver = tf.contrib.eager.Saver(dbm.params() + enc.params())
average_loss_rbm = 0.
average_loss_enc = 0.
...
energy_pos_list, ent_enc_list = [], []
rbm_loss_list, enc_loss_list, loss_list = [], [], []
...
for step in range(1, num_steps + 1):
    ...
    if step % ver_steps == 0:
        ...
        loss_list.append(-(average_loss_enc).numpy())
        end = time.time()
        print("Step: %05d, rbm loss = %08f, ais log Z = %08f, enc loss = %08f, "
              "ais ll = %08f, ais train = %08f, ais test = %08f, best test = %08f, time = %08f\n" %
              (step, average_loss_rbm, ais_log_z, average_loss_enc, ais_ll, ais_ll_train, ais_ll_test, best_test_ll, end - start))
        try:
            with open(logfile, 'a') as f:
                f.write("Step: %05d, rbm loss = %08f, ais log Z = %08f, enc loss = %08f, "
                        "ais ll = %08f, ais train = %08f, ais test = %08f, best test = %08f, time = %08f\n" %
                        (step, average_loss_rbm, ais_log_z, average_loss_enc, ais_ll, ais_ll_train, ais_ll_test, best_test_ll, end - start))
    ...
    try:
        ...
        if step % vis_steps == 0:
            # plotting curves
            to_plot_list = [loss_list, ais_ll_list, energy_pos_list, rbm_loss_list, ais_log_z_list]
            ...
            legend = plt.legend(('neg_loss', 'ais LL', 'pos free energy', 'neg free energy', 'rbm loss', 'ais log Z'))
            plt.grid(True)
